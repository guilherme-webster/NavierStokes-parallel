...
int allocate_memory(double*** u, double*** v, double*** p, double*** res, double*** RHS, double*** F, double*** G, int i_max, int j_max) {
   ...
   *u = (double**) calloc(i_max + 2, sizeof(double*)); /* u values on the right edges of the ghost cells are not needed */
   ...
   *G = (double**) calloc(i_max + 2, sizeof(double*));
   for (i = 0; i < i_max + 2; i++)
   {
       ...
       (*v)[i] = (double*) calloc(j_max + 2, sizeof(double)); /* v values on the top edges of the ghost cells are not needed */
       if (i < i_max + 1) {
